...
*/
var d_date, c_year, c_month, t_year, t_month, t_day, v_year, v_month, v_day, ca_sep, ca_id, ca_interval, ca_timeout, ca_callback;
var today = new Date();
...
function ca_show(id, obj, sep, callback) {
	Eh();
...
function ca_select(year, month, day) {
	...
	day = ca_padzero(day);
	var setday = year+ca_sep+month+ca_sep+day;
	var inputday = Dd(ca_id).getAttribute('data-value');
	var s = (new Date(setday.replace(/-/g,"\/"))) - (new Date(inputday.replace(/-/g,"\/")));
	var chaday = s/1000/60/60/24;
	if(chaday >= 0){
		Dd(ca_id).value = year+ca_sep+month+ca_sep+day;
		if(typeof(ca_callback)=='function'){
			ca_callback();
		}
		ca_hide();
	}
