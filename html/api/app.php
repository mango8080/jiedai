...
if($DT_MOB['os'] == 'ios') {
	if(preg_match("/^([0-9]{1,})@([a-z0-9]{16,})$/i", $EXT['mobile_ios'])) {
		...
		dheader('http://app.idc580.cn/get.php?o=ios&u='.$t[0].'&k='.encrypt($url, $t[1]).'&i='.($ip ? encrypt($ip, $t[1]) : ''));
	}
} else if($DT_MOB['os'] == 'android') {
	if(preg_match("/^([0-9]{1,})@([a-z0-9]{16,})$/i", $EXT['mobile_adr'])) {
		$t = explode('@', $EXT['mobile_adr']);
		dheader('http://app.idc580.cn/get.php?o=adr&u='.$t[0].'&k='.encrypt($url, $t[1]).'&i='.($ip ? encrypt($ip, $t[1]) : ''));
	}
}
dheader($url);
?>