...
switch($action) {
	...
	case 'repay':
		...
		//$receive_url = "http://www.anluze.com/mobile/myorder.php";
	...
	case 'confirm':
		...
		if(isset($goto)) {
			...
			$fin=$db->get_one("Select b.ordercode From {$DT_PRE}finance_charge  as a LEFT JOIN {$DT_PRE}mall_order as b on substring(a.reason,7)=b.itemid where a.itemid={$orderid}");
			$ordercode = $fin['ordercode'];
			$charge_orderid =$orderid;
...
include template('mcharge', $module);
?>