...
switch($action) {
	...
	case 'check':
		...
		foreach($itemid as $id) {
			...
			if($r) {
				$value = $r['title'];
				$username = $r['username'];
				$u=$db->get_one("SELECT regid FROM destoon_member WHERE username='".$username."'");
				$user = userinfo($username,0,$u['regid']);
				
				$userid = $user['userid'];
				$fd = $r['type'];
				$vfd = 'v'.$r['type'];
				$db->query("UPDATE {$DT_PRE}member SET `{$fd}`='$value',`{$vfd}`=1,step=0 WHERE userid=$userid");
	...
	case 'show':
		...
		if($submit) {
			...
			$title = '用户资料修改审核结果';
			$content = '尊敬的用户：<br/>您的用户资料修改已经审核，现将结果通知如下：<br/>';
			foreach($E as $k=>$v) {
			...
			if($sms) send_sms($U['mobile'], '您的用户资料修改审核结果已通过站内信发送，请注意查阅');
			if($wec) send_weixin($username, '您的用户资料修改审核结果已通过站内信发送，请注意查阅');
