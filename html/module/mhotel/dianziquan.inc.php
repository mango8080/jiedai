...
if($action == 'update') {
	...
	$condition = "a.buyer='$_username'";
	if($keyword) $condition .= " AND b.title LIKE '%$keyword%'";
	...
	$r = $db->get_one("SELECT COUNT(*) AS num FROM `{$DT_PRE}voucher_order` as a left join `{$DT_PRE}sell_23` as b on a.voucherid=b.itemid WHERE $condition");
	$pages = pages($r['num'], $page, $pagesize);		
	$lists = array();
	$result = $db->query("SELECT a.*,b.title,b.price FROM `{$DT_PRE}voucher_order` as a left join `{$DT_PRE}sell_23` as b on a.voucherid=b.itemid WHERE $condition ORDER BY a.itemid DESC LIMIT $offset,$pagesize");
	$amount = $fee = $money = 0;
...
include template('hotel_dianziquan', $module);
?>