...
if($submit) {
	...
	$r = $db->get_one("SELECT username,passport,step FROM {$DT_PRE}member  WHERE $option='$username' and regid=6");

	if($r) {
		$username = $r['username'];
		$passport = $r['passport'];
		$step=$r['step'];
		set_cookie('step', $step, $DT_TIME + 30*86400);
	} else {
	...
	$user = $do->login($username, $password, $cookietime,'',6);
	if($user) {
	...
	include template('hotel_login', $module);
}
?>