...
switch($action) {
	...
	case 'sync':
		...
		if($arr['next_openid'] == '' || $arr['next_openid'] == $next_openid) {
			msg('用户同步成功，开始同步用户资料...', '?moduleid='.$moduleid.'&file='.$file.'&action=sync_user');
		} else {
			msg('已同步 '.$num.' 位用户'.progress(1, $num, $arr['total']), '?moduleid='.$moduleid.'&file='.$file.'&action='.$action.'&num='.$num.'&next_openid='.$arr['next_openid']);
		}		
	break;
	case 'sync_user':
		require DT_ROOT.'/api/weixin/init.inc.php';
	...
	case 'user':
		...
		$sfields = array('按条件', '用户名', '昵称', '城市', '省份', '国籍', '语言');
	...
	break;
	default:
		$sfields = array('按条件', '消息内容', '微信昵称', '用户名', '网站编辑');
