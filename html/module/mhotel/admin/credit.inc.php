...
switch($action) {
	...
	case 'add':
		if($submit) {
			...
			foreach($usernames as $username) {
				...
				if(!$r) {
					$error .= '<br/>用户['.$username.']不存在';
					continue;
				}
				if(!$type && $r['credit'] < abs($amount)) {
					$error .= '<br/>用户['.$username.']'.$DT['credit_name'].'不足，当前'.$DT['credit_name'].'为:'.$r['credit'];
					continue;
				}
				$reason or $reason = '奖励';
				$note or $note = '手工';
			...
			if($error) message('操作成功 '.$success.' 位用户，发生以下错误：'.$error);
	...
	break;
	default:
		$sfields = array('按条件', '用户名', '金额', '事由', '备注', '操作人');
