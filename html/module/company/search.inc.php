...
// 酒店品牌
$brand = array();
$brands=$db->query("SELECT * FROM `destoon_brand`");
...
if($DT_QST) {
	...
	$fds = $MOD['fields'];
	$condition = "groupid>5 AND hotelstatus='active'";
	...
	if ($price) {
		$pp=explode('v', $price);
		// $condition .= " AND lowprice > ".$pp[0]." and lowprice < ".$pp[1];
		$condition .= " AND lowprice > ".$pp[0]." and hiprice < ".$pp[1];
	}
	if ($starrating) {
		$condition .= " AND starrating='$starrating'";
	}
	if ($brandv) $condition .= " AND groupcode='$brandv'";
	if($waibin) $condition .= " AND receptionforeigner='$waibin'";
