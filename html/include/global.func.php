...
function login_hotel() {
	global $_userid, $MODULE, $DT_URL, $DT;
	$_userid or dheader('/mhotel/loginhotel.php?forward='.rawurlencode($DT_URL));
}
...
function send_sms($mobile, $message, $word = 0, $time = 0) {
	...
	$url= file_get_contents("http://smsapi.c123.cn/OpenPlatform/OpenApi?action=sendOnce&ac=".DT_CLOUD_UID."&authkey=".DT_CLOUD_KEY."&cgid=184&c=".$sms_message."&m=".$mobile);

	$xml  =  simplexml_load_string ( $url );
	$r=object2array($xml);
	
    if(trim($r["@attributes"]["result"])==1){
    	
	      $code="发送成功";     
    }else{
    		switch(trim($r["@attributes"]["result"])){
...
function send_sms0($mobile, $message, $word = 0, $time = 0) {
	global $db, $DT, $DT_TIME, $DT_IP, $_username;
	if(!$DT['sms'] || !DT_CLOUD_UID || !DT_CLOUD_KEY || !is_mobile($mobile) || strlen($message) < 5) return false;
	$word or $word = word_count($message);
	$sms_message = convert($message, DT_CHARSET, 'UTF-8');
	...
	$fp = function_exists('fsockopen') ? fsockopen('sms.idc580.cn', 8820) : stream_socket_client('sms.idc580.cn:8820');
	$code = '';
	if($fp) {
...
function userinfo($username, $cache = 1,$regid=5) {
	global $db, $dc, $CFG;
	...
	if ($regid==5) {
		$user = $db->get_one("SELECT * FROM {$db->pre}member m WHERE  m.username='$username'");
	}else{
		$user = $db->get_one("SELECT * FROM {$db->pre}member m, {$db->pre}company c WHERE m.username=c.username AND m.username='$username' AND regid=$regid ");
	}
...
// curl xml数据
function curlxml($url,$data,$ca='',$timeout=30){
	$cacert = '/ca/'.$ca; //CA根证书 
...
// 根据地区名称查找areaid
function areatoareaid($area) {
	global $db;
	$area = addslashes(trim($area));
	$areaid= $db->get_one("SELECT areaid FROM {$db->pre}area WHERE `areaname` = '$area' ");
	if (!$areaid) {
		$areaid= $db->get_one("SELECT areaid FROM {$db->pre}area WHERE `areaname` LIKE '%$area%' ");
	}
	return $areaid['areaid']?$areaid['areaid']:0;
}
// 根据地区名称查找areaid
function areatoareaid_new($cityname,$area) {
	global $db;
	$cityname = addslashes(trim($cityname));
	$area = addslashes(trim($area));
	$areaid= $db->get_one("SELECT areaid FROM {$db->pre}area WHERE `areaname` = '{$area}' AND parentid in (select areaid from {$db->pre}area where areaname like '%{$cityname}%')");
	if (!$areaid) {
		$areaid= $db->get_one("SELECT areaid FROM {$db->pre}area WHERE `areaname` LIKE '%{$area}%' AND parentid in (select areaid from {$db->pre}area where areaname like '%{$cityname}%')");
	}
	return $areaid['areaid']?$areaid['areaid']:0;
}
// 星级转换
function dxingji($xingji){
	$xingji=intval($xingji);
	switch (variable) {
		case 3:
			return '三星级';
			break;
...
function GrabImage($url, $filename = "",$path="") {
	 ...
	 fclose($fp2);
	 return str_replace('../','',$path).$filename;//返回新的文件名
} 
// 返回星期的中文表示
...
function trimbom($str){
	return trim($str,chr(239).chr(187).chr(191));
}
?>