...
// if($_userid) dheader('/mhotel/');
require DT_ROOT.'/module/mhotel/common.inc.php';
if(isset($read)) exit(include template('hotel_agreement', $module));
...
if($submit) {
	...
	if($do->add($post)) {
		...
		// $forward = 'goto.php?action=register_success&username='.$username.'&auth='.encrypt('LOGIN|'.$username.'|'.$post['password'].'|'.$DT_TIME, DT_KEY.'LOGIN').'&forward='.urlencode('reghotel2.php');
		
		$forward ='login.php?submit=1&auth='.encrypt('LOGIN|'.$username.'|'.$post['password'].'|'.$DT_TIME, DT_KEY.'LOGIN').'&forward='.urlencode('reghotel2.php');
	...
	include template('reghotel', $module);
}
?>