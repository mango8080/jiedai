...
<tr>
<td class="tl">用户便签默认值</td>
<td><textarea name="setting[usernote]" style="width:96%;height:30px;overflow:visible;"><?php echo $usernote;?></textarea><?php tips('用户便签没有填写时，默认显示此值');?>
</td>
</tr>
<tr>
<td class="tl">用户资料修改需审核</td>
<td>
<?php
$ECK = array(
	'thumb' => '形象图片',
...
<td>
<input type="text" size="3" name="setting[maxtouser]" value="<?php echo $maxtouser;?>"/> 位用户<?php tips('最小填1，例如填5则表示，同一信件一次最多可以同时发送给5位用户');?>
</td>
</tr>
<tr>
<td class="tl">发送站内短信启用验证码</td>
<td>
<input type="radio" name="setting[captcha_sendmessage]" value="2" <?php if($captcha_sendmessage == 2) echo 'checked';?>> 继承用户组设置&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<td>
<input type="radio" name="setting[alert_check]" value="2" <?php if($alert_check == 2) echo 'checked';?>> 继承用户组设置&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<td>
<input type="radio" name="setting[news_check]" value="2" <?php if($news_check == 2) echo 'checked';?>> 继承用户组设置&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<td>
<input type="radio" name="setting[page_check]" value="2" <?php if($page_check == 2) echo 'checked';?>> 继承用户组设置&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<td>
<input type="radio" name="setting[credit_check]" value="2" <?php if($credit_check == 2) echo 'checked';?>> 继承用户组设置&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<td>
<input type="radio" name="setting[link_check]" value="2" <?php if($link_check == 2) echo 'checked';?>> 继承用户组设置&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<td><input type="text" size="60" name="setting[pay_url]" value="<?php echo $pay_url;?>"/><?php tips('如果未启用用户在线充值，则系统自动调转至此地址查看普通付款方式。建议用扩展功能的单网页建立');?></td>
</tr>
<tr>
<td class="tl">用户提现</td>
<td>
...
<td><input type="text" size="5" name="setting[deposit]" value="<?php echo $deposit;?>"/><?php tips('例如设置为1000，用户每次增加保证金最少为1000且必须是1000的倍数，例如2000、5000、10000，最小值为100');?></td>
</tr>
<tr>
...
<div class="tt">用户整合</div>
<table cellpadding="2" cellspacing="1" class="tb">
<tr>
<td class="tl">启用用户整合</td>
<td>
...
<td><input name="setting[passport_url]" type="text" size="50" value="<?php echo $passport_url;?>"/><?php tips('整合程序接口地址 例如:http://bbs.idc580.cn 结尾不要带斜线');?></td>
</tr>
<tr>
<td class="tl">整合密钥</td>
...
<td><input name="setting[uc_api]" type="text" size="50" value="<?php echo $uc_api;?>" id="uc_api"/><?php tips('整合程序接口地址 例如:http://bbs.idc580.cn 结尾不要带斜线');?></td>
</tr>
<tr>
<td class="tl">主机IP</td>
...
<tr>
<td class="tl">论坛用户自动激活</td>
<td>
<input type="radio" name="setting[uc_bbs]" value="0" <?php if(!$uc_bbs) echo 'checked';?>/> 关闭&nbsp;&nbsp;
<input type="radio" name="setting[uc_bbs]" value="1" <?php if($uc_bbs) echo 'checked';?>/> 开启 <?php tips('此项可以在用户注册后自动激活论坛帐号，但仅适用于使用DZX2以上版本的论坛，且论坛与UC安装在同一数据库，且整合方式为MySQL连接，请确认你的整合符合上述条件，否则请勿开启');?>
</td>
</tr>
<tr>
<td class="tl">论坛表前缀</td>
<td><input name="setting[uc_bbspre]" type="text" size="10" value="<?php echo $uc_bbspre;?>" id="uc_bbspre"/> <?php tips('如果开启自动激活，此项必须填写。注意：填写错误可能导致用户无法注册！<br/>默认的表前缀为pre_，具体请参考论坛数据库配置文件');?></td>
</tr>
</tbody>
</table>
