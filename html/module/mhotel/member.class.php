...
class member {
	...
	function set_member($member) {
		...
		$member['jiameng'] = isset($member['jiameng']) ? intval($member['jiameng']) : 0;
		$member['thumb'] = isset($member['thumb']) ? str_replace(DT_PATH,'',$member['thumb']) : '';
		if ($member['map']) {
			$map=explode(',', $member['map']);
			$member['longitude']=$map[0];
			$member['latitude']=$map[1];
		}
	...
	function email_exists($email) {
		$condition = "email='$email' and regid=6";
	...
	function mobile_exists($mobile,$regid=6) {
		$condition = "mobile='$mobile' AND regid=$regid";
		if($this->userid) $condition .= " AND userid!=$this->userid";
		return $this->db->get_one("SELECT userid FROM {$this->table_member} WHERE {$condition}");
	}
	...
	function add($member) {
		...
		$member['sound'] = 1;
		$member['jiameng'] = 1;
		$member_fields = array('username','company','passport', 'password','payword','email','sound','gender','truename','mobile','msn','qq','ali','skype','department','career','groupid','regid','areaid','edittime','inviter','passsalt', 'paysalt','cardcode','step');
		$company_fields = array('username','groupid','company','type','catid','catids','areaid', 'mode','capital','regunit','size','regyear','sell','buy','business','telephone','fax','mail','address','postcode','homepage','introduce','thumb','keyword','linkurl','policies','totalrooms','starrating','jiameng');
	...
	function edit($member)	{
		...
		$member_fields = array('company','passport','sound','email','msn','qq','ali','skype','gender','truename','mobile','department','career','groupid','areaid', 'edittime','black','bank','banktype','branch','account','vemail','vmobile','vbank','vtruename','vcompany','vtrade','trade','support','inviter','cardcode');
		$company_fields = array('company','type','areaid', 'catid','catids','business','mode','regyear','regunit','capital','size','address','postcode','telephone','fax','mail','homepage','sell','buy','introduce','thumb','keyword','linkurl','groupid','domain','icp','validated','validator','validtime','skin','template','policies','totalrooms','starrating','longitude','latitude','apigroup');
	...
	function login($login_username, $login_password, $login_cookietime = 0, $admin = false,$regid=6) {
		...
		$user = userinfo($login_username, 0,$regid);
		if(!$user) {
