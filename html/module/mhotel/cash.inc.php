...
switch($action) {
	...
	case 'confirm':
		...
		if($submit) {
			...
			$db->query("INSERT INTO {$DT_PRE}finance_cash (username,bank,banktype,branch,account,truename,amount,fee,addtime,ip,title,zitemid) VALUES ('$_username','$member[bank]','$member[banktype]','$member[branch]','$member[account]','$name','$money','$fee','$DT_TIME','$DT_IP','$title','$zitemid')");
			$cid = $db->insert_id();
...
include template('hotel_cash', $module);
?>