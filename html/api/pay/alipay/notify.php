...
log_write($verify_result, 'abc5', 1);
if($verify_result) {
	//$out_trade_no = intval($out_trade_no);
  
   $mallorder = $db->get_one("SELECT itemid FROM {$DT_PRE}mall_order WHERE ordercode='$out_trade_no'");
   $trade='trade|'.$mallorder['itemid'];
   $r = $db->get_one("SELECT * FROM {$DT_PRE}finance_charge WHERE reason='$trade'");
	if($r) {
		if($r['status'] == 0) {
