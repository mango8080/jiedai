...
switch($action) {
	...
	case 'refund':
		...
		if($submit) {
			...
			if($msg || $eml || $sms || $wec) {
				...
				$content = '尊敬的用户：<br/>您的[团购订单]'.$td['title'].'(单号:'.$td['itemid'].')'.$result.'！<br/>';
	...
	break;
	case 'express':
		$sfields = array('按条件', '房型名称', '卖家', '买家', '订单金额', '密码', '买家名称', '买家地址', '买家邮编', '买家电话', '买家手机', '发货快递', '发货单号', '备注');
	...
	default:
		$sfields = array('按条件', '房型名称', '卖家', '买家', '订单金额', '密码', '买家名称', '买家地址', '买家邮编', '买家电话', '买家手机', '发货快递', '发货单号', '备注');
		$dfields = array('title', 'title', 'seller', 'buyer', 'amount', 'password', 'buyer_name', 'buyer_address', 'buyer_postcode', 'buyer_phone', 'buyer_mobile', 'send_type', 'send_no', 'note');
		$sorder  = array('排序方式', '下单时间降序', '下单时间升序', '更新时间降序', '更新时间升序', '房型单价降序', '房型单价升序', '购买数量降序', '购买数量升序', '订单金额降序', '订单金额升序');
