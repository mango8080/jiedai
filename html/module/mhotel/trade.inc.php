...
if($action == 'update') {
	...
	switch($step) {
		...
		case 'detail'://订单详情
		if ($json==1) {
			$td['outtime']=timetodate($td['outtime'], 0);
			$td['intime']=timetodate($td['intime'], 0);
			$td['status'] = $_status[$td['status']];
			exit(json_encode($td));
		}else{
			...
			include template('hotel_order_detail', $module);
			exit;
		}
		...
		case 'print'://订单打印
			...
			include template('hotel_trade_print', $module);
			exit;
		break;
		case 'express'://快递追踪
		...
		case 'pay'://买家付款
			...
			if($td['status'] == 6 ) message('您选择的是到店付款');
			$quanlist = array();
			if ($td['voucherid']) {
				// 电子券
				$quan=$db->query("SELECT * FROM {$DT_PRE}sell_23 where itemid in(".$td['voucherid'].")");
				while($r = $db->fetch_array($quan)){
					$quanlist[$r['title']]=$r['price'];
				}
			}
...
}else if($action == 'ruzhu'){
	// if($td['status'] != 7 || !$td['cod'] || !$td['send_time'] || $td['seller'] != $_username) message($L['trade_msg_deny']);
	$td = $db->get_one("SELECT a.buyer_name,a.buyer_mobile,a.title,b.company FROM {$table} as a Left Join {$DT_PRE}company as b on a.seller=b.username WHERE a.itemid=$itemid");
	$sms = new Sms();
	$mobile=$td['buyer_mobile'];
	$sms_note='{"name":"'.$td['buyer_name'].'","hotelname":"'.$td['company'].'","intime":"'.date("Y-m-d H:i",$DT_TIME).'"}';
	$db->query("UPDATE {$table} SET status=4,updatetime=$DT_TIME WHERE itemid=$itemid");
	$status = $sms->send_sms($mobile, $sms_note, 'SMS_34840267');
	//交易成功
	dmsg('入住成功', $forward);
}else if($action == 'queren'){
	 //echo $ac;exit;
	$qrm=$_GET["qrm"];
	$td = $db->get_one("SELECT a.buyer_name,a.buyer_mobile,a.title,b.company FROM {$table} as a Left Join {$DT_PRE}company as b on a.seller=b.username WHERE a.itemid=$itemid");
	$sms = new Sms();
	$mobile=$td['buyer_mobile'];
	$sms_note='{"name":"'.$td['buyer_name'].'","hotelname":"'.$td['company'].'","roomtype":"'.$td['title'].'"}';
	
	$status1=$db->get_one("SELECT * FROM {$table} WHERE itemid=$itemid");
	//echo $status1['status'];exit;
	if ($ac==4) {
		$db->query("UPDATE {$table} SET access=4,status=9,updatetime=$DT_TIME WHERE itemid=$itemid");
		$sms_templete="SMS_34935274";
	}else{
		if($status1['status']==0){
		$db->query("UPDATE {$table} SET access=1,status=1,updatetime=$DT_TIME,qrm=$qrm WHERE itemid=$itemid");
		$sms_note='{"name":"'.$td['buyer_name'].'","hotelname":"'.$td['company'].'","roomtype":"'.$td['title'].'"}';
		$sms_templete="SMS_35055353";
		}else{
			$db->query("UPDATE {$table} SET access=1,updatetime=$DT_TIME,qrm=$qrm WHERE itemid=$itemid");
		$sms_note='{"name":"'.$td['buyer_name'].'","hotelname":"'.$td['company'].'","roomtype":"'.$td['title'].'"}';
		$sms_templete="SMS_35055353";
		}
	}
	$status = $sms->send_sms($mobile, $sms_note, $sms_templete);
	...
	if($itemid) $condition .= " AND itemid=$itemid";
	if($mallid) $condition .= " AND mallid=$mallid";
	if($buyer) $condition .= " AND buyer_name LIKE '%$buyer%'";
	//echo $condition;exit;
	if($cod) $condition .= " AND cod=1";
	if($dingdanhao) $condition .= " AND ordercode='$dingdanhao'";
	if(in_array($nav, array(0,1,2,3,5,6))) {
		$condition .= " AND status=$nav";
	} else if($nav == 4) {
	...
	$result = $db->query("SELECT * FROM {$table} WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
	//var_dump("SELECT * FROM {$table} WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");exit;
	//echo $DT_PRE;exit;
	$amount = $fee = $money = 0;
	while($r = $db->fetch_array($result)) {
...
include template('hotel_trade', $module);
?>