...
if($submit) {
	...
	if($do->add($post)) {
		...
		$forward = 'goto.php?action=register_success&username='.$username.'&auth='.encrypt('LOGIN|'.$username.'|'.$post['password'].'|'.$DT_TIME, DT_KEY.'LOGIN').'&forward=reghotel2.php';
		// var_dump($forward);exit;
		// $forward ='reghotel2.php?auth='.encrypt($username);
