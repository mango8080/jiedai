...
- 内容支持变量，用户资料保存于$user数组<br/>
- 例 {$user[username]} 表示用户名<br/>
- 例 {$user[company]} 表示公司名<br/>
- 如果是给非用户发送短信，请不要使用变量<br/>
<?php if(!$DT['sms'] || !DT_CLOUD_UID || !DT_CLOUD_KEY) { ?>
<span class="f_red">- 注意：无法发送，未设置发送参数</span> <a href="?file=setting&tab=7" class="t">点此设置</a><br/>
<?php } else { ?>

<span class="f_red">
- 由于政策原因，并非所有内容都可以正常发送...<a href="<?php echo DT_PATH;?>api/redirect.php?url=http://help.idc580.cn/use/29.html%23faq" target="_blank" class="t">了解详情</a><br/>
