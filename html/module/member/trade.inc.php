...
if($action == 'update') {
	...
	if($cod) $condition .= " AND a.cod=1";
	if(in_array($nav, array(0,1,2,3,5,6))) {
		$condition .= " AND status=$nav";
	} else if($nav == 4) {
		$condition .= " AND status=$nav AND seller_star=0";
	}
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} as a WHERE $condition");
	$pages = pages($r['num'], $page, $pagesize);		
	$lists = array();
	$result = $db->query("SELECT  a.*,b.company,b.username from {$DT_PRE}mall_order  a left join {$DT_PRE}company  b on a.seller=b.username WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
	$amount = $fee = $money = 0;
	while($r = $db->fetch_array($result)) {
