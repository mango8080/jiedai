...
用户ID：<input type="text" name="uid" value="<?php echo $uid;?>" size="10"/>&nbsp;
</td>
</tr>
</table>
</form>
...
<td><a href="?moduleid=2&action=edit&userid=<?php echo $v['userid'];?>"><img src="admin/image/edit.png" width="16" height="16" title="修改用户[<?php echo $v['username'];?>]资料" alt=""/></a>&nbsp;
...
<?php } ?>
<div class="btns">
<input type="submit" value=" 删除公司 " class="btn" onclick="if(confirm('确定要删除选中用户吗？系统将删除选中用户所有信息，此操作将不可撤销')){this.form.action='?moduleid=2&action=delete'}else{return false;}"/>&nbsp;
<input type="submit" value=" 禁止访问 " class="btn" onclick="if(confirm('确定要禁止选中用户访问吗？')){this.form.action='?moduleid=2&action=move&groupids=2'}else{return false;}"/>&nbsp;
...
<input type="submit" value=" 移动至 " class="btn" onclick="if(Dd('mgroupid').value==0){alert('请选择用户组');Dd('mgroupid').focus();return false;}this.form.action='?moduleid=2&action=move';"/> <?php echo group_select('groupid', '用户组', 0, 'id="mgroupid"');?>&nbsp;
