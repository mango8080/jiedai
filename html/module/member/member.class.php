...
class member {
	...
	function email_exists($email) {
		$condition = "email='$email' and regid=5";
	...
	function mobile_exists($mobile,$regid=5) {
	...
	function add($member) {
		...
		$member_fields = array('username','company','passport', 'password','payword','email','sound','gender','truename','mobile','msn','qq','ali','skype','department','career','groupid','regid','areaid','edittime','inviter','passsalt', 'paysalt','danwei','jinghao','jingzhong','cardcode','danwei','shangxian');
        ...
        $company_sqlv = substr($company_sqlv, 1);//echo "INSERT INTO {$this->table_member} ($member_sqlk,regip,regtime,loginip,logintime)  VALUES ($member_sqlv,'$DT_IP','$DT_TIME','$DT_IP','$DT_TIME')";
	...
	function edit($member)	{
		...
		$company_fields = array('company','type','areaid', 'catid','catids','business','mode','regyear','regunit','capital','size','address','postcode','telephone','fax','mail','homepage','sell','buy','introduce','thumb','keyword','linkurl','groupid','domain','icp','validated','validator','validtime','skin','template','policies','zhekou');
	...
	function login($login_username, $login_password, $login_cookietime = 0, $admin = false,$regid=5) {
		...
		if($user['groupid'] == 2) return $this->_($L['member_login_member_ban']);		if($user['groupid'] == 4) return $this->_($L['send_password_checking']);
		$userid = $user['userid'];
