...
switch($action) {
	...
	case 'edit':
		...
		if($submit) {
			...
			if($status == 3) {
				//
				$db->query("UPDATE `destoon_zhangdan` set status=3,ispay=1 WHERE itemid='$zitemid'");
			} else if($status == 2 || $status == 1) {
				$note or msg('请填写原因备注');
				money_add($item['username'], $money);
				money_record($item['username'], $money, '站内', 'system', '提现失败', '流水号:'.$itemid);
				$db->query("UPDATE `destoon_zhangdan` set status=4 WHERE itemid='$zitemid'");
			} else {
				msg();
			}
	...
	break;
	default:
		$sfields = array('按条件', '用户名', '金额', '手续费', '开户银行', '开户网点', '收款户名', '收款帐号', '备注', '受理人');
