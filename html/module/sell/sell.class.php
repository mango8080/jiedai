...
class sell {
		...
		if($catids) {
			...
			if($CATS) {
				...
				foreach($lists as $k=>$v) {
					...
					$kucun = $this->db->get_one("SELECT COUNT(*) AS num FROM destoon_sell_dianzijuan WHERE categoryid={$v['itemid']} and state<>1");
					$xiaoshou = $this->db->get_one("SELECT COUNT(*) AS num FROM destoon_sell_dianzijuan WHERE categoryid={$v['itemid']} and state=1");

					$lists[$k]['xiaoshou'] = $xiaoshou['num'];
					$lists[$k]['kucun'] = $kucun['num'];
	...
	function add2($post) {
        ...
        $item = $this->db->get_one("SELECT itemid FROM {$this->table} WHERE addtime=".$post['addtime']);
        ...
        $sqlk = substr($sqlk, 1);
        $sqlv = substr($sqlv, 1);
		$this->db->query("INSERT INTO destoon_sell_dianzijuan ($sqlk,state) VALUES ($sqlv,0)");
		$this->juanid = $this->db->insert_id();
		$content_table = content_table($this->moduleid, $this->juanid, $this->split, $this->table_data);
