...
class mall {
    ...
    function __construct($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','mycatid','areaid','level','title','style','fee','introduce','brand','price','step','amount','unit','thumb','thumb1','thumb2','tag','status','hits','username','editor','addtime','adddate','edittime','editdate','ip','template','linkurl','filepath','elite','note','company','truename','telephone','mobile','address','email','msn','qq','ali','skype','n1','n2','n3','v1','v2','v3','express_1','express_name_1','fee_start_1','fee_step_1','express_2','express_name_2','fee_start_2','fee_step_2','express_3','express_name_3','fee_start_3','fee_step_3','cod','hiprice','breakfastdesc','netdesc','bedtype','service','area','floor','jihuo','hiprice','shiprice','shifromdate','shitodate');
    }
	...
	function set($post) {
		...
		$post['thumb']=str_replace(DT_PATH, '', $post['thumb']);
		$post['thumb1']=str_replace(DT_PATH, '', $post['thumb1']);
		$post['thumb2']=str_replace(DT_PATH, '', $post['thumb2']);
		...
		$post['shifromdate'] = strtotime($post['shifromdate']);
		$post['shitodate'] = strtotime($post['shitodate']);
	...
	function check($itemid) {
		...
		if(is_array($itemid)) {
			...
			$this->tohtml($itemid);
			// 取得该酒店的所有房型最低价
			$lowprices=$this->db->get_one("SELECT price from {$this->table} WHERE username='".$item['username']."' AND status=3 ORDER BY price asc");
			$this->db->query("UPDATE destoon_company SET lowprice=".$lowprices['price']."  WHERE username='".$item['username']."'");
			return true;
		}
	}
