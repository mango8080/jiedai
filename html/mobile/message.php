...
$_userid or dheader('login.php?forward='.urlencode('message.php?action='.$action));$NAME = $L['message_type'];
switch($action) {
	case 'send':
		if(isset($_POST['ok'])) {
	...
	case 'show':
		if($itemid) {
			...
			$head_title = $title.$DT['seo_delimiter'].$L['message_title'].$DT['seo_delimiter'].$head_title;			$content=str_replace('width="700"', 'width="100%"', $content);
		} else {			
