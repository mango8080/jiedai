...
show_menu($menus);
?>
<form method="post" action="?" onsubmit = "return check_box();">
...
<tr>
<td class="tl">网站地址</td>
<td><input name="config[url]" type="text" value="<?php echo $url;?>" size="40"/><?php tips('请添写完整URL地址,例如http://www.idc580.cn/<br/>注意以 / 结尾');?></td>
</tr>
<tr>
...
<tr>
<td class="tl">购买第三方服务限额</td>
<td><input name="setting[dianzijuan]" id="dianzijuan" type="text" value="<?php echo $dianzijuan;?>" size="10"/>元
<input name="setting[dianzijuanbfb]" id="dianzijuanbfb" type="text" value="<?php echo $dianzijuanbfb;?>" size="10"/>%
</td>
</tr>
<tr>
<td class="tl">购买电子券说明</td>
<td>
<textarea name="setting[voucher_dis]" style="width:500px;height:50px;"><?php echo $voucher_dis; ?></textarea>
</td>
</tr>
...
<td><input name="config[com_domain]" type="text" value="<?php echo $com_domain;?>" size="30"/> <?php tips('如果填写 .idc580.cn 同时需要将域名泛解析 *.idc580.cn 指向服务器IP，并且在服务器端绑定泛域名至 网站根目录 或者 网站根目录/company 目录，生成的主页形式为username.idc580.cn<br/>如果填写 i.idc580.cn 同时需要将域名泛解析 i.idc580.cn 指向服务器IP，并且在服务器端绑定域名至网站根目录/company 目录，生成的主页形式为i.idc580.cn/username/(注：此方式必须支持伪静态)');?></td>
</tr>
<tr>
...
<input type="radio" name="setting[com_www]" value="0"  <?php if(!$com_www){ ?>checked <?php } ?>/> 关闭 <?php tips('例如二级域名为sell.idc580.cn<br/>添加www后为 www.sell.idc580.cn');?>
</td>
</tr>
<tr>
<td class="tl">用户顶级域名Rewrite</td>
<td>
<input type="radio" name="config[com_rewrite]" value="1"  <?php if($com_rewrite){ ?>checked <?php } ?>/> 开启&nbsp;&nbsp;
<input type="radio" name="config[com_rewrite]" value="0"  <?php if(!$com_rewrite){ ?>checked <?php } ?>/> 关闭 <?php tips('部分服务器可能无法开启用户绑定的顶级域名Rewrite，如果无法开启，可在此关闭，以免出现打不开页面的情况，此项仅针对用户顶级域名，不影响其他页面Rewrite');?>
</td>
</tr>
...
<tr>
<td class="tl">用户在线保持时间</td>
<td><input type="text" name="setting[online]" value="<?php echo $online;?>" size="5"/> 秒<?php tips('超过此时间未有活动的用户将视为离线');?></td>
</tr>

<tr>
<td class="tl">定时更新用户新消息</td>
<td><input type="text" name="setting[pushtime]" value="<?php echo $pushtime;?>" size="5"/> 秒<?php tips('当用户停留在前台页面时，每隔一段时间，系统自动发送一次服务器请求，以更新用户站内信、新对话、购物车数量，以便用户及时收到新消息，填0为关闭，此项会增加服务器压力，建议设置30秒以上');?></td>
</tr>
...
<td><input name="setting[remote_url]" type="text" value="<?php echo $remote_url;?>" size="60"/><?php tips('例如 http://static.idc580.cn/，注意以 / 结尾');?></td>
</tr>
<tr> 
...
<td><input name="setting[check_hour]" type="text" value="<?php echo $check_hour;?>" size="60"/><?php tips('此项针对前台用户发布信息，留空表示不限制，建议设置为工作人员下班时间。时间设置请参考允许登录后台的时段 Tips');?></td>
</tr>
<tr>
...
<tr>
<td class="tl">信任域名</td>
<td><input name="setting[safe_domain]" type="text" value="<?php echo $safe_domain;?>" size="60"/><?php tips('不填写则默认为当前域名<br/>多个域名请用|分开 例如idc580.cn|destoon.cn');?></td>
</tr>
<tr>
...
<td><input name="config[cookie_domain]" type="text" value="<?php echo $cookie_domain;?>" size="20"/><?php tips('例如要保证顶级域名idc580.cn所有二级域名均可正常登录注销，则填写.idc580.cn(注意顶级域名前加.)');?></td>
</tr>
<tr>
...
<input type="radio" name="setting[message_weixin]" value="0"  <?php if(!$message_weixin) echo 'checked';?>/> 关闭<?php tips('仅在开启了微信公众号，且用户绑定了微信，且用户在48小时内打开过公众号才能推送成功');?>
</td>
</tr>
</tbody>
</table>
...
<tr>
<td class="tl">短信单价</td>
<td><input name="setting[sms_fee]" type="text" value="<?php echo $sms_fee;?>" size="5"/> <?php echo $DT['money_unit'];?>/条 <?php tips('此项针对用户收费');?></td> 
</tr>
<tr>
<td class="tl">每日上限</td>
<td><input name="setting[sms_max]" type="text" value="<?php echo $sms_max;?>" size="5"/> 条 <?php tips('特指用户注册、找回密码、手机验证等需要发送验证码场景，同一手机号码或用户每日最大发送数量，填0为不限制，建议填5左右的数字，以免恶意发送');?></td> 
</tr>
...
<tr style="display:none;">
<td class="tl">成功标识</td>
<td><input name="setting[sms_ok]" type="text" value="<?php echo $sms_ok;?>" size="10"/> <?php tips('短信发送成功标识字符，系统根据此字符确定是否扣除用户短信余额');?></td> 
</tr>
<tr>
<td class="tl">短信内容签名</td>
<td><input name="setting[sms_sign]" type="text" value="<?php echo $sms_sign;?>" size="30"/> <?php tips('将显示在短信内容结尾，以便用户识别，请尽量简短，正确的格式为【签名】，例如 【某某网】。包含签名的短信会被运营商认为是更正规的短信，从而进入更快的发送通道');?></td> 
</tr>
</tbody>
</table>
...
<script>
	function check_box(){
		   var dianzijuan =document.getElementById('dianzijuan');
		   var dianzijuanbfb =document.getElementById("dianzijuanbfb");
		  	if((dianzijuan.value!="")&&(dianzijuanbfb.value!="")){
		  		alert("第三方服务限额只能填一个");
		  		dianzijuan.focus();
			    return false;
		  	}
...
<script type="text/javascript" src="http://www.idc580.cn/sms.php?action=get&uid=<?php echo DT_CLOUD_UID;?>&key=<?php echo md5(DT_CLOUD_KEY.'|'.DT_CLOUD_UID);?>"></script>
